...
try {
  ...
  try {
    execSync("npm install html-minifier-terser clean-css --save-dev", {
      stdio: "inherit",
    });
    ...
    console.error(
      "Please run: npm install html-minifier-terser clean-css --save-dev"
    );
    process.exit(1);
  }
}
...
jsFilesToObfuscate.forEach((file) => {
  ...
  if (fs.existsSync(distFile)) {
    ...
    try {
      ...
      // Obfuscate
      execSync(
        `javascript-obfuscator ${distFile} --output ${distFile} ${obfuscatorOptions}`,
        {
          stdio: "pipe",
        }
      );
...
htmlFiles.forEach((file) => {
  try {
    ...
    const minifiedSize = (Buffer.byteLength(minified, "utf8") / 1024).toFixed(
      2
    );
    ...
    console.log(
      `    ${originalSize} KB → ${minifiedSize} KB (${savings}% smaller)\n`
    );
  } catch (err) {
...
cssFiles.forEach((file) => {
  try {
    ...
    if (output.errors.length > 0) {
      ...
      const minifiedSize = (
        Buffer.byteLength(output.styles, "utf8") / 1024
      ).toFixed(2);
      const savings = ((1 - minifiedSize / originalSize) * 100).toFixed(1);
